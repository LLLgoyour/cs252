...
def read_cat_csv(filepath):
    ...
    # Read raw rows as list of lists of strings
    raw_rows = [] 
    with open(filepath, "r") as f: 
        for line in f:
            line = line.strip()
    ...
    # Encode rows into ints
    coded_rows = []
    for row in raw_rows:
        # pad shorter rows with empty strings
        if len(row) < len(var_names):
            row = row + [''] * (len(var_names) - len(row))
        code_row = []
        for j, var in enumerate(var_names):
            val = row[j]
            if val == '':
                val = 'Missing'
            if val not in levels[var]:
                levels[var].append(val)
            code_row.append(levels[var].index(val))
        coded_rows.append(code_row)

    return coded_rows, levels