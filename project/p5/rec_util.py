...
def user_item_matrix(df, num_users, num_movies):
    ...
    user_ids = df['userId'].to_numpy(dtype = int)
    movie_ids = df['movieId'].to_numpy(dtype = int)
    ratings = df['rating'].to_numpy(dtype = float)
    A[user_ids, movie_ids] = ratings
    return A
...
def get_top_rated_movies(A, movie_map, user_id, num_top_rated):
    ...
    '''
    rated_movie_ids = np.where(A[user_id] > 0)[0]
    rated_movie_ratings = A[user_id, rated_movie_ids]
    top_idx = np.argsort(-rated_movie_ratings, kind='stable')[:num_top_rated]
    return [movie_map[movie_id] for movie_id in rated_movie_ids[top_idx]]
...
def get_top_recommendations(A, rec_model, movie_map, user_id, num_top_recommendations):
    ...
    ndarray. shape=(num_top_recommendations,). Floats representing the predicted numerical ratings of the top
        recommended movies.
    '''
    unrated_movie_ids = np.where(A[user_id] == 0)[0]
    user_ids = np.full(unrated_movie_ids.shape, user_id)
    pred_ratings = rec_model.predict_user_item_ratings(user_ids, unrated_movie_ids)
    top_idx = np.argsort(-pred_ratings, kind='stable')[:num_top_recommendations]
    top_movie_ids = unrated_movie_ids[top_idx]
    top_movie_titles = [movie_map[movie_id] for movie_id in top_movie_ids]
    top_movie_ratings = pred_ratings[top_idx]
    return top_movie_titles, top_movie_ratings
...
def plot_recommendations(user_id, top_movie_titles, top_movie_ratings, figsize=(5, 7)):
    ...
    for bar, rating in zip(bars, top_movie_ratings):
        ax.text(rating, bar.get_y() + bar.get_height() / 2, f'{rating:.2f}', va='center', ha='left')
