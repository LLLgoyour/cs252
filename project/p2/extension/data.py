...
class Data:
    ...
    def __init__(self, filepath=None, headers=None, data=None, header2col=None, cats2levels=None):
        ...
        self.data = None if data is None else np.copy(data)
        self.header2col = {} if header2col is None else dict(header2col)
        self.cats2levels = {} if cats2levels is None else dict(cats2levels)
        
        if self.headers and not self.header2col:
    ...
    def _strip_newline(self, s):
        return s.rstrip("\n\r")

    def _split_csv_row(self, line):
        # Basic CSV splitting for this class project.
        # Assumes no quoted commas inside fields.
        return [cell.strip() for cell in self._strip_newline(line).split(",")]

    def _pad_row(self, row, target_len):
        if len(row) < target_len:
            row = row + [""] * (target_len - len(row))
        return row

    def _parse_numeric_cell(self, text):
        if text == "":
            return float("nan")
        return float(text)

    def _parse_categorical_cell(self, header, text):
        if text == "":
            text = "Missing"
        levels = self.cats2levels[header]
    ...
    def read(self, filepath):
        ...
        if len(lines) < 2:
            raise ValueError("Either a header row or a type row is missing")

        raw_headers = self._split_csv_row(lines[0])
        raw_types = [t.lower() for t in self._split_csv_row(lines[1])]

        if len(raw_types) != len(raw_headers):
            raise ValueError("Two row length does not match")

        valid_types = {"numeric", "categorical", "string", "date"}
        if any(t not in valid_types for t in raw_types):
            raise ValueError("Missing a valid type row as row 2")

        keep_cols = []
        keep_types = []

        for col_i, dtype in enumerate(raw_types):
            if dtype == "numeric" or dtype == "categorical":
                keep_cols.append(col_i)
                keep_types.append(dtype)

                header = raw_headers[col_i]
                self.header2col[header] = len(self.headers)
        ...
        for line in lines[2:]:
            ...
            entries = self._split_csv_row(line)
            entries = self._pad_row(entries, len(raw_headers))

            row_vals = []
            for new_col, orig_col in enumerate(keep_cols):
                dtype = keep_types[new_col]
                header = self.headers[new_col]
                text = entries[orig_col] if orig_col < len(entries) else ""

                if dtype == "numeric":
                    row_vals.append(self._parse_numeric_cell(text))
                else:
                    row_vals.append(self._parse_categorical_cell(header, text))

            self.data.append(row_vals)
    ...
    def __str__(self):
        ...
        out = '-------------------------------\n'
        out += f'{self.filepath} ({num_samples}x{num_dims})\n'
        out += 'Headers:\n'
        out += '  ' + '\t'.join(self.headers) + '\n'
        out += '-------------------------------\n'

        rows_to_show = min(5, num_samples)
        if num_samples > 5:
            out += f'Showing first {rows_to_show}/{num_samples} rows.\n'

        for row_i in range(rows_to_show):
            row_str = '\t'.join([str(self.data[row_i, col_i]) for col_i in range(num_dims)])
            out += row_str + '\n'
    ...
    def head(self):
        ...
        if self.data is None:
            return None
        rows = min(5, self.get_num_samples())
        return np.copy(self.data[:rows, :])
    ...
    def limit_samples(self, start_row, end_row):
        ...
        if self.data is None:
            return
        self.data = np.copy(self.data[start_row:end_row, :])
    ...
    def shuffle(self, inds):
        ...
        self.data = np.copy(self.data[inds, :])
    ...
    def select_data(self, headers, rows=[]):
        ...
        if rows is None or len(rows) == 0:
            return np.copy(self.data[:, col_inds])
        rows_arr = np.array(rows, dtype=int)
        return np.copy(self.data[np.ix_(rows_arr, col_inds)])